    ...
    <>
      <motion.nav initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.2 }} className={`fixed top-0 pt-3 left-0 right-0 z-50 px-6 py-0 transition-all duration-300 ${scrolled || menuOpen ? "bg-white/90 backdrop-blur-md shadow-sm" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto flex items-center justify-between pr-0 lg:pr-20">
          <a className="flex items-center h-[100px]">
          ...
          <div className="flex items-center gap-3">
            {/* Language switcher visible on mobile next to burger */}
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            ...
            <motion.div key="drawer" initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25, ease: "easeOut" }} className="fixed top-[103px] left-0 right-0 z-40 bg-white/95 backdrop-blur-md shadow-lg lg:hidden border-t border-gray-100">
              <ul className="flex flex-col px-6 py-4">
                {links.map((link, i) => (
                  <motion.li key={link.href} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05, duration: 0.2 }}>
                    <a href={link.href} onClick={() => setMenuOpen(false)} className="flex items-center gap-3 py-3.5 text-[#6B6B6B] hover:text-[#1A1A1A] font-medium text-base border-b border-gray-100 last:border-0 transition-colors group">
